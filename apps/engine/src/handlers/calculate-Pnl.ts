import { mullInternal, type EngineUser } from "@perp-v1-boilerplate/commons";
...
export function recalculatePnl(user: EngineUser) {
  for (const positon of user.positions) {
    ...
    if (positon.type === "LONG") {
      positon.pnL = mullInternal(markPrice - positon.averagePrice, positon.qty);
    } else {
      positon.pnL = mullInternal(positon.averagePrice - markPrice, positon.qty);
    }
  }
}
