...
import { toInternalPrice } from "@perp-v1-boilerplate/commons";
      ...
      for (const stream of raw) {
        for (const { id, message } of stream.messages) {
          try {
            ...
            const markPrice = toInternalPrice(Number(message.markPrice));
            const indexPrice = toInternalPrice(Number(message.indexPrice));
